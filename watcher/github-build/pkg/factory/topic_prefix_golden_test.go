...
// by CreateTriggerBuildCheckCommandSender (and, by construction, the
// equivalent CreateKafkaCreateSender/CreateCommandConsumer wiring, since all
// three build a cdb.CommandObjectSender from the same TopicPrefix over the
// same lib.GithubBuildV1SchemaID{Group: "maintainer", Kind: "githubbuild",
...
var _ = Describe("CreateTriggerBuildCheckCommandSender topic naming (golden)", func() {
	sendAndCaptureTopic := func(topicPrefix string) string {
		...
		err := sender.SendCommand(context.Background(), command.TriggerBuildCheckCommand{})
	...
	It("prefixes with develop for the dev stage", func() {
		...
		).To(Equal("develop-maintainer-githubbuild-v1-request"))
	})

	It("prefixes with master for the prod stage", func() {
		Expect(sendAndCaptureTopic("master")).To(Equal("master-maintainer-githubbuild-v1-request"))
	})

	It("has no prefix when TopicPrefix is empty", func() {
		Expect(sendAndCaptureTopic("")).To(Equal("maintainer-githubbuild-v1-request"))
	})
})
