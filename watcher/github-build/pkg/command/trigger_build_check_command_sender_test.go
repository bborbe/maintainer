...
var _ = Describe("TriggerBuildCheckCommandSender.SendCommand", func() {
	var (
		ctx      context.Context
		fakeCDB  *cdbmocks.CDBCommandObjectSender
		sender   command.TriggerBuildCheckCommandSender
		validCmd command.TriggerBuildCheckCommand
	)

	BeforeEach(func() {
		ctx = context.Background()
		fakeCDB = new(cdbmocks.CDBCommandObjectSender)
		sender = command.NewTriggerBuildCheckCommandSender(
	...
	Context("valid command", func() {
		It("publishes one CommandObject with the correct operation and SchemaID", func() {
			...
			Expect(obj.SchemaID).To(Equal(lib.GithubBuildV1SchemaID))
			Expect(obj.Command.Operation).To(Equal(command.TriggerBuildCheckCommandOperation))
		})
	})
	...
	Context("Kafka publish fails", func() {
		It("returns a wrapped Kafka error", func() {
			...
			Expect(err.Error()).To(ContainSubstring("send TriggerBuildCheckCommand to Kafka"))
	...
	Context("downstream is fed the correct command bytes", func() {
		It("populates the event payload from the command via base.ParseEvent", func() {
			...
			// constructed must round-trip back to the original TriggerBuildCheckCommand.
			fakeCDB.SendCommandObjectReturns(nil)
			...
			var roundTripped command.TriggerBuildCheckCommand
