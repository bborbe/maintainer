...
var _ = Describe("computePRTitle", func() {
	DescribeTable(
		...
		func(provider, owner, repo string, number int, sha, title string, maxSlug, maxTitle int, want string) {
			Expect(
				computePRTitle(provider, owner, repo, number, sha, title, maxSlug, maxTitle),
			).To(Equal(want))
		},
