...
- feat(watcher/github-release): replace `GetAutoReleaseConfig` / `.dark-factory/config.yml` trust gate with `GetMaintainerConfig` / `.maintainer.yaml: release.autoRelease`; flip `AutoReleaseFilter` semantics from "skip when true" to "pass when true" (positive opt-in gate); remove all legacy config code and tests; update README and decision-chains doc; counterfeiter mock regenerated
