...
6. **TaskPublisher.BuildCreateCommand** produces a `task.CreateCommand` whose `Frontmatter` matches the Phase 1 frozen contract per [[Agent Task File Contract]]: `task_type: github-release`, `assignee: github-releaser-agent`, `phase: planning`, `status: in_progress`, `stage` (from config), `task_identifier` (UUID5), `title` (`Release <owner>/<repo> at <sha[:7]>` — slash + " at " literal, per Phase 1 vault evidence; the controller transforms `/` → `-` and strips ` at ` when slugifying for the on-disk filename, but the title field itself MUST carry the slash + at form), `repo` (`owner/name`), `clone_url` (`git@github.com:owner/name.git`), `ref` (full HeadSHA), `current_version`. Body is the operator-readable header only (title + version + HEAD + changelog URL + repo link) — no `## Unreleased` bullets embedded.
