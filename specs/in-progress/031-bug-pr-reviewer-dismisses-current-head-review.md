...
- [ ] The dismissal-filter predicate uses `r.CommitID != headSHA` (not `==`) — evidence: `grep -rnE 'r\.CommitID\s*(==|!=)\s*headSHA' agent/pr-reviewer/pkg/githubposter/` returns exactly one match in non-test source and that match uses `!=`. (Path-resilient: survives a future rename within the package.)
...
- [ ] No existing test asserts the old "current-head review is dismissed" behavior — evidence: review test files under `agent/pr-reviewer/pkg/githubposter/` (`grep -rn 'commit_id\|CommitID' agent/pr-reviewer/pkg/githubposter/*_test.go`) and confirm no assertion (in any phrasing — struct literal, fixture builder, helper) places a review whose `commit_id` equals the test-case head SHA into the expected dismissal output. Implementor records the grep output + a written one-line confirmation in the PR description.
