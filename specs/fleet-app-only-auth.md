...
- [ ] Each service's "auth not configured" startup error names the App env vars and not `GH_TOKEN` — evidence: `grep -rni 'APP_ID' <service>/main.go` finds the App env var named in the service's startup/error path for each of the 4 services (the exact phrasing differs across the 3 resolver shapes — match `APP_ID` case-insensitively in the error string, NOT a fixed literal like "set APP_ID"), AND `grep -rni 'GH_TOKEN' agent/pr-reviewer watcher/github-pr watcher/github-build watcher/github-release --include=*.go | grep -iv 'env\["GH_TOKEN"\]\|forward\|subprocess'` returns zero PAT-*input* references (the only surviving `GH_TOKEN` is the resolved-IAT *output* forwarding, e.g. `env["GH_TOKEN"]=` in pr-reviewer — that stays).
